...
# parameters
k = 10 # number of bits per symbol
n = 1000 # frame length
Ka = 2 # number of active device
...
P1_vec = np.linspace(1e-9, P, 50)
epsilon = np.zeros_like(P1_vec)

# Number of samples for CDF evaluation
Niq = 100
...
for pp in tqdm.tqdm(range(len(P1_vec))):
    ...
    P1 = P1_vec[pp]
    # compute of p0
    p0 = math.comb(Ka, 2) / M + Ka * sp.gammaincc(n * P / P1, n)
    # compute R1, R2
    R1 = 1 / n * np.log(float(M)) - 1 / (n * t) * sp.gammaln(Ka - t + 1)
    ...
    for II in range(Niq):
        ...
        it = n * np.log(1 + P1) + (np.sum(np.abs(np.tile(Zi, (Ka, 1)) + codebook) ** 2, axis=1) / (1 + P1) - np.sum(np.abs(np.tile(Zi, (Ka, 1)))**2, axis=1))
        It[II] = np.min(it)
    ...
    # epsilon[pp] = t_vec[0] / Ka * np.min([pt[0], qt]) +
    epsilon[pp] = np.sum(t_vec[1:] / Ka * pt[1:]) + p0
